...
public class SubscriptionService {
    ...
    public String subscribe(String subscriberUrl, int opsNo) {
        ...
        log.info("Submitting ONDC /subscribe to {} with subscriber_id: {}, ops_no: {}",
                registryUrl, ondcProperties.getSubscriberId(), opsNo);

        try {
            ondcRestTemplate.postForEntity(registryUrl, payload, Map.class);
            log.info("ONDC /subscribe submitted successfully. Awaiting /on_subscribe challenge. requestId: {}",
                    requestId);
        } catch (Exception e) {
