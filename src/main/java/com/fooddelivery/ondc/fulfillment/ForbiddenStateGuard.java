...
public class ForbiddenStateGuard {
    ...
    private static final Set<String> FORBIDDEN_STATES = Set.of(
            "In-transit", "At-destination-hub", "Out-for-delivery"
    );

    /** Valid transitions: from → set of allowed next states */
    private static final Map<OndcFulfillmentState, Set<OndcFulfillmentState>> VALID_TRANSITIONS = Map.of(
            OndcFulfillmentState.PENDING, Set.of(
                    OndcFulfillmentState.PACKED,
                    OndcFulfillmentState.AGENT_ASSIGNED),
            OndcFulfillmentState.PACKED, Set.of(
                    OndcFulfillmentState.AGENT_ASSIGNED,
                    OndcFulfillmentState.OUT_FOR_PICKUP,
                    OndcFulfillmentState.ORDER_PICKED_UP),
            OndcFulfillmentState.AGENT_ASSIGNED, Set.of(
                    OndcFulfillmentState.OUT_FOR_PICKUP,
                    OndcFulfillmentState.AT_PICKUP,
                    OndcFulfillmentState.ORDER_PICKED_UP),
            OndcFulfillmentState.OUT_FOR_PICKUP, Set.of(
                    OndcFulfillmentState.AT_PICKUP,
                    OndcFulfillmentState.ORDER_PICKED_UP),
            OndcFulfillmentState.AT_PICKUP, Set.of(
                    OndcFulfillmentState.ORDER_PICKED_UP),
            OndcFulfillmentState.ORDER_PICKED_UP, Set.of(
                    OndcFulfillmentState.AT_DELIVERY,
                    OndcFulfillmentState.ORDER_DELIVERED),
            OndcFulfillmentState.AT_DELIVERY, Set.of(
                    OndcFulfillmentState.ORDER_DELIVERED),
            OndcFulfillmentState.ORDER_DELIVERED, Set.of() // terminal state
    );
    ...
    public void validateTransition(OndcFulfillmentState from, OndcFulfillmentState to) {
        ...
        if (allowed == null || !allowed.contains(to)) {
            throw new IllegalStateException(
                    "Invalid fulfillment state transition: " + from.getOndcValue() +
                            " → " + to.getOndcValue());
        }
