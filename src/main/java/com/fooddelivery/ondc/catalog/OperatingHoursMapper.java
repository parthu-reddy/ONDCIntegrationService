...
public class OperatingHoursMapper {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OperatingHoursMapper.class);
    ...
    public java.util.Map<String, Object> mapToOndcTimeRange(String operatingHours) {
        ...
        return java.util.Map.of("range", java.util.Map.of("start", parts[0].trim().length() == 5 ? parts[0].trim() + ":00" : parts[0].trim(), "end", parts[1].trim().length() == 5 ? parts[1].trim() + ":00" : parts[1].trim()));
    }
}
