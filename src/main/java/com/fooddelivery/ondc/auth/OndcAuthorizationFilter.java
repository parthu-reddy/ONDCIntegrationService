...
// After RawBodyCachingFilter (HIGHEST_PRECEDENCE)
/**
...
public class OndcAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (headerToVerify == null || headerToVerify.isBlank()) {
            ...
            response.getWriter().write("{\"message\":{\"ack\":{\"status\":\"NACK\"}}," + "\"error\":{\"type\":\"CONTEXT-ERROR\",\"code\":\"10001\"," + "\"message\":\"Missing Authorization header\"}}");
            return;
        }
        try {
    ...
    private boolean isBecknEndpoint(String uri) {
        return uri.matches("^/(search|select|init|confirm|cancel|status|track|update|rating|" + "on_search|on_select|on_init|on_confirm|on_cancel|on_status|on_track|" + "search_inc|on_search_inc|" + "settle|on_settle|recon|on_recon|receiver_recon|on_receiver_recon)$");
    }

    @java.lang.SuppressWarnings("all")
    public OndcAuthorizationFilter(final SignatureVerificationService signatureVerificationService) {
