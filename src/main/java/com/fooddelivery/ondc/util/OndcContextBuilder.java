...
public class OndcContextBuilder {
    ...
    public OndcContext buildBppResponseContext(OndcContext incomingContext, String action) {
        return OndcContext.builder().domain(ondcProperties.getDomain()).action(action).country(ondcProperties.getCountry()).city(incomingContext.getCity() != null ? incomingContext.getCity() : ondcProperties.getCity()).coreVersion("1.2.0").bapId(incomingContext.getBapId()).bapUri(incomingContext.getBapUri()).bppId(ondcProperties.getSubscriberId()).bppUri(incomingContext.getBppUri()).transactionId(incomingContext.getTransactionId()).messageId(incomingContext.getMessageId()).timestamp(DateTimeFormatter.ISO_INSTANT.format(Instant.now())).build();
    }
    ...
    public OndcContext buildBapRequestContext(String action, String bppId, String bppUri) {
        ...
        OndcContext.builder().domain(ondcProperties.getDomain()).action(action).country(ondcProperties.getCountry()).city(ondcProperties.getCity()).coreVersion("1.2.0").bapId(ondcProperties.getSubscriberId()).bapUri(ondcProperties.getSubscriberUrl()).bppId(bppId).bppUri(bppUri).transactionId(UUID.randomUUID().toString()).messageId(UUID.randomUUID().toString()).timestamp(DateTimeFormatter.ISO_INSTANT.format(Instant.now())).build();
    }

    @java.lang.SuppressWarnings("all")
    public OndcContextBuilder(final OndcProperties ondcProperties) {
