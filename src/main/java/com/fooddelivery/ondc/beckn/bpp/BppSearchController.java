...
public class BppSearchController {
    ...
    public ResponseEntity<OndcAckResponse> search(@RequestBody OndcRequest request) {
        log.info("Received /search from BAP: {}, transaction_id: {}", request.getContext().getBapId(), request.getContext().getTransactionId());
        // Validate mandatory fields
        schemaValidator.validateRequest(request);
        schemaValidator.validateSearchContext(request.getContext());
        // Log transaction for audit
        OndcTransaction txn = OndcTransaction.builder().transactionId(request.getContext().getTransactionId()).messageId(request.getContext().getMessageId()).action("search").bapId(request.getContext().getBapId()).bppId(request.getContext().getBppId()).state("RECEIVED").build();
        transactionRepository.save(txn);
        // Publish to Kafka for async processing (catalog lookup + on_search callback)
        kafkaTemplate.send(TOPIC_ONDC_SEARCH_REQUEST, request.getContext().getTransactionId(), serializeRequest(request));
    ...
    @java.lang.SuppressWarnings("all")
    public BppSearchController(final OndcSchemaValidator schemaValidator, final OndcTransactionRepository transactionRepository, final KafkaTemplate<String, String> kafkaTemplate) {
