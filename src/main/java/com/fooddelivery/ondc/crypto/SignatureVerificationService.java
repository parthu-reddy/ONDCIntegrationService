...
public class SignatureVerificationService {
    ...
    public void verifySignature(String authHeader, byte[] rawBody) {
        ...
        if (expires < now - tolerance) {
            throw new OndcSignatureException("Signature expired. Expires: " + expires + ", Current: " + now);
        }
        if (created > now + tolerance) {
            throw new OndcSignatureException("Signature created in the future. Created: " + created + ", Current: " + now);
        }
        ...
        if (!valid) {
            log.error("ONDC signature verification FAILED for subscriber: {}, keyId: {}", subscriberId, uniqueKeyId);
            throw new OndcSignatureException("Ed25519 signature verification failed for subscriber: " + subscriberId);
        }
        log.debug("ONDC signature verified successfully for subscriber: {}", subscriberId);
    }

    @java.lang.SuppressWarnings("all")
    public SignatureVerificationService(final Ed25519KeyManager ed25519KeyManager, final BlakeDigestService blakeDigestService, final SignatureService signatureService, final RegistryLookupService registryLookupService, final OndcProperties ondcProperties) {
